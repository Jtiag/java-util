...
public class FileUtil {
	...
	/**
	 * 获取classpath中的文件内容
	 * @param filePath 相对classpath的文件路径
	 * @return
	 */
	public static String getClassPathFileContent(String filePath){
		...
		try {
			...
			return InputStreamUtils.InputStreamTOString(is,"UTF-8");
		} catch (Exception e) {
			e.printStackTrace();
		} finally{
			if(is!=null){
	...
	public static void readClassPathResourceToFile(String filePath,File file){
		InputStream is=null;
		try {
			is = new ClassPathResource(filePath).getInputStream();
